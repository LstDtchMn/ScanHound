...
# ── Phase 3 red baseline: MOVED ───────────────────────────────────────
#
# The two xfail(strict=True) placeholders that lived here described the
# measured authority failures against a local simulation of the unfenced code.
# Phase 3 landed the real fence, so they were replaced by tests that exercise
# the production path instead of a stand-in: see tests/test_publication_fence.py.

...
# ======================================================================
# Dead / duplicate link handling in the LinkGrabber
# ======================================================================

def _fake_device(*, grabber_links=(), grabber_packages=(), downloads_links=()):
    """A JD device whose linkgrabber answers with real lists, not MagicMocks.

    The send path iterates these results, so a bare MagicMock would raise on
    iteration and be swallowed by the best-effort guards — hiding whatever the
    test meant to assert.
    """
    device = MagicMock()
    device.linkgrabber.is_collecting.return_value = False
    device.linkgrabber.query_packages.return_value = list(grabber_packages)
    device.linkgrabber.query_links.return_value = list(grabber_links)
    device.downloads.query_links.return_value = list(downloads_links)
    return device


class TestLinkIsDead:
    def test_offline_is_dead(self):
        assert DownloadService._link_is_dead("OFFLINE") is True
        assert DownloadService._link_is_dead("offline") is True

    def test_online_is_not_dead(self):
        assert DownloadService._link_is_dead("ONLINE") is False

    @pytest.mark.parametrize("value", ["UNKNOWN", "TEMP_UNKNOWN", "", None])
    def test_unconcluded_check_is_not_dead(self, value):
        """An inconclusive check must never strand a link.

        UNKNOWN/TEMP_UNKNOWN mean JD could not decide, not that the link is
        bad. Treating them as dead would recreate the stall for every host
        JDownloader cannot probe.
        """
        assert DownloadService._link_is_dead(value) is False


class TestReleaseOnlineLinks:
    PKG = [{"name": "Some Movie (2024)", "uuid": 1}]

    def test_releases_good_links_and_leaves_dead_behind(self):
        device = _fake_device(
            grabber_packages=self.PKG,
            grabber_links=[
                {"uuid": 10, "packageUUID": 1, "availability": "ONLINE"},
                {"uuid": 11, "packageUUID": 1, "availability": "OFFLINE"},
                {"uuid": 12, "packageUUID": 1, "availability": "ONLINE"},
            ],
        )
        svc = _make_service()
        summary = svc._release_online_links(device, "Some Movie (2024)")
        assert summary == {"moved": 2, "dead": 1, "released": True}
        # ONLY the live children are confirmed; the dead one stays put.
        device.linkgrabber.move_to_downloadlist.assert_called_once_with([10, 12], [])

    def test_no_dead_links_leaves_autostart_alone(self):
        """Nothing is holding the package, so this must not churn it."""
        device = _fake_device(
            grabber_packages=self.PKG,
            grabber_links=[{"uuid": 10, "packageUUID": 1, "availability": "ONLINE"}],
        )
        svc = _make_service()
        summary = svc._release_online_links(device, "Some Movie (2024)")
        assert summary == {"moved": 0, "dead": 0, "released": False}
        device.linkgrabber.move_to_downloadlist.assert_not_called()

    def test_all_dead_moves_nothing(self):
        device = _fake_device(
            grabber_packages=self.PKG,
            grabber_links=[
                {"uuid": 10, "packageUUID": 1, "availability": "OFFLINE"},
                {"uuid": 11, "packageUUID": 1, "availability": "OFFLINE"},
            ],
        )
        svc = _make_service()
        summary = svc._release_online_links(device, "Some Movie (2024)")
        assert summary == {"moved": 0, "dead": 2, "released": False}
        device.linkgrabber.move_to_downloadlist.assert_not_called()

    def test_ignores_other_packages(self):
        """A dead link in someone else's package must not be counted here."""
        device = _fake_device(
            grabber_packages=self.PKG,
            grabber_links=[
                {"uuid": 10, "packageUUID": 1, "availability": "ONLINE"},
                {"uuid": 99, "packageUUID": 2, "availability": "OFFLINE"},
            ],
        )
        svc = _make_service()
        assert svc._release_online_links(device, "Some Movie (2024)") == {
            "moved": 0, "dead": 0, "released": False}

    def test_matches_package_through_jd_name_sanitization(self):
        """JD rewrites ':' to ';', so an exact name match would find nothing."""
        device = _fake_device(
            grabber_packages=[{"name": "Movie; The Sequel", "uuid": 1}],
            grabber_links=[
                {"uuid": 10, "packageUUID": 1, "availability": "ONLINE"},
                {"uuid": 11, "packageUUID": 1, "availability": "OFFLINE"},
            ],
        ...
        svc = _make_service()
        summary = svc._release_online_links(device, "Movie: The Sequel")
        assert summary == {"moved": 1, "dead": 1, "released": True}

    def test_unknown_package_returns_none(self):
        device = _fake_device(grabber_packages=[{"name": "Other", "uuid": 7}])
        svc = _make_service()
        assert svc._release_online_links(device, "Some Movie (2024)") is None


class TestFilterKnownLinks:
    def test_drops_links_already_in_the_linkgrabber(self):
        device = _fake_device(
            grabber_links=[{"url": "https://host.example/a"}])
        svc = _make_service()
        out = svc._filter_known_links(
            device, ["https://host.example/a", "https://host.example/c"])
        assert out == ["https://host.example/c"]

    def test_a_link_already_downloading_is_NOT_filtered(self):
        """A deliberate re-grab must not be silently gutted.

        The pipeline's regrab action passes force=True to bypass download_item's
        dedup gates, but this guard runs BELOW that gate. Filtering against the
        downloads list would drop every link of an already-downloaded package
        and no-op the grab while still reporting success.
        """
        device = _fake_device(
            grabber_links=[],
            downloads_links=[{"url": "https://host.example/a"}],
        )
        svc = _make_service()
        assert svc._filter_known_links(device, ["https://host.example/a"]) == [
            "https://host.example/a"]
        device.downloads.query_links.assert_not_called()

    def test_matches_despite_cosmetic_url_differences(self):
        """JD stores a different scheme/www/trailing slash than we scraped."""
        device = _fake_device(
            grabber_links=[{"url": "http://www.host.example/a/"}])
        svc = _make_service()
        assert svc._filter_known_links(device, ["https://host.example/a"]) == []

    def test_keeps_everything_when_jd_is_empty(self):
        device = _fake_device()
        svc = _make_service()
        assert svc._filter_known_links(device, ["https://host.example/a"]) == [
            "https://host.example/a"]
...
class TestApiSendSkipsDeadAndDuplicateLinks:
    CONFIG = {
        "jd_method": "api",
        "jd_email": "u@e.com",
        "jd_password": "p",
    ...
    def _svc_with(self, device):
        svc = _make_service(config=dict(self.CONFIG))
        svc._connect_jd_device = MagicMock(return_value=device)
        return svc

    def test_duplicate_links_are_not_resent(self):
        """Every link already in JD -> nothing re-sent, but still a success.

        Re-sending is what raises JD's duplicate warning, and that warning is
        what blocks the package in the desktop app.
        """
        device = _fake_device(
            grabber_links=[{"url": "https://host.example/a"}])
        svc = self._svc_with(device)
        outcome = {}
        assert svc.send_to_jdownloader(
            ["https://host.example/a"], "Pkg", outcome=outcome) is True
        device.linkgrabber.add_links.assert_not_called()
        assert outcome == {"transport": "api", "confirmed": True}

    def test_only_unknown_links_are_sent(self):
        device = _fake_device(
            grabber_links=[{"url": "https://host.example/a"}],
            grabber_packages=[{"name": "Pkg", "uuid": 1}],
        )
        svc = self._svc_with(device)
        assert svc.send_to_jdownloader(
            ["https://host.example/a", "https://host.example/b"], "Pkg") is True
        payload = device.linkgrabber.add_links.call_args[0][0][0]
        assert payload["links"] == "https://host.example/b"

    def test_dead_link_does_not_hold_the_package(self):
        """The whole point: one dead mirror must not stall the good links."""
        device = _fake_device(
            grabber_packages=[{"name": "Pkg", "uuid": 1}],
            grabber_links=[
                {"uuid": 10, "packageUUID": 1, "availability": "ONLINE"},
        ...
        svc = self._svc_with(device)
        assert svc.send_to_jdownloader(["https://host.example/new"], "Pkg") is True
        device.linkgrabber.move_to_downloadlist.assert_called_once_with([10], [])
        device.downloadcontroller.start_downloads.assert_called_once()

    def test_release_failure_never_fails_a_delivered_grab(self):
        """The links are already delivered; a cleanup error must not undo that."""
        device = _fake_device(grabber_packages=[{"name": "Pkg", "uuid": 1}])
        device.linkgrabber.query_links.side_effect = RuntimeError("JD went away")
        svc = self._svc_with(device)
        outcome = {}
        assert svc.send_to_jdownloader(
            ["https://host.example/new"], "Pkg", outcome=outcome) is True
        assert outcome == {"transport": "api", "confirmed": True}
        device.linkgrabber.add_links.assert_called_once()

    def test_dupe_filter_failure_falls_back_to_sending_everything(self):
        device = _fake_device(grabber_packages=[{"name": "Pkg", "uuid": 1}])
        device.linkgrabber.query_links.side_effect = RuntimeError("JD unhappy")
        svc = self._svc_with(device)
        assert svc.send_to_jdownloader(["https://host.example/new"], "Pkg") is True
        payload = device.linkgrabber.add_links.call_args[0][0][0]
        assert payload["links"] == "https://host.example/new"
...
class TestGetDriver:
    ...
    @patch("backend.download_service._ensure_selenium")
    @patch("backend.download_service.launch_browser")
    def test_retries_chrome_launch_then_succeeds(self, mock_launch, mock_ensure):
        ...
        # Driver creation now goes through browser_adapter.launch_browser, which
        # returns a (driver, browser_status) tuple.
        svc = _make_service()
        good = MagicMock()
        mock_launch.side_effect = [Exception("session not created"), (good, {})]
        svc._kill_stale_chrome = MagicMock()

        result = svc.get_driver()
        assert result is good
        assert mock_launch.call_count == 2       # retried once
        svc._kill_stale_chrome.assert_called_once()  # reaped between attempts

    @patch("backend.download_service.time.sleep", MagicMock())
    @patch("backend.download_service._ensure_selenium")
    @patch("backend.download_service.launch_browser")
    def test_chrome_launch_failure_raises_after_retries(self, mock_launch, mock_ensure):
        svc = _make_service()
        mock_launch.side_effect = Exception("cannot connect to chrome")
        svc._kill_stale_chrome = MagicMock()

        with pytest.raises(Exception, match="cannot connect to chrome"):
            svc.get_driver()
        assert mock_launch.call_count == 3       # bounded to 3 attempts

    @patch("backend.download_service._ensure_selenium")
    @patch("backend.download_service.launch_browser")
    def test_creates_new_driver_if_no_cached(self, mock_launch, mock_ensure):
        svc = _make_service()
        mock_driver = MagicMock()
    ...
    @patch("backend.download_service._ensure_selenium")
    @patch("backend.download_service.launch_browser")
    def test_recreates_driver_if_stale(self, mock_launch, mock_ensure):
        svc = _make_service()
        ...
        mock_launch.return_value = (new_driver, {})

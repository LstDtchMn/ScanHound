...
from backend.rename.process_control import ProcessCancelled
...
def test_cancelled_base_probe_keeps_item_retryable(tmp_path, monkeypatch):
    media = tmp_path / "movie.mkv"
    media.write_bytes(b"generated test media")
    db = DatabaseManager(str(tmp_path / "inventory.sqlite"))
    entered = threading.Event()

    def cancellable_probe(_path, *, cancel_requested=None, **_kwargs):
        entered.set()
        assert callable(cancel_requested)
        deadline = time.monotonic() + 2
        while time.monotonic() < deadline:
            ...
            time.sleep(0.01)
        raise AssertionError("base probe did not observe cancellation")

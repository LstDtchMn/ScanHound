...
**Separate finding, filed, not folded in (HDE-6 candidate):** the behaviour that masked this leak is a latent production hazard. `configure()` wipes a live cooldown whenever it is handed a different config or db object, and `backend/detail_scraper.py:~95` reconfigures the coordinator with `getattr(parent_app, "db", None)` through app bridges that define no `db`; constructing a `DetailScraper` therefore clears any live Turnstile cooldown and detaches health persistence until something reconfigures with the real db. Bounded today by startup order (the scanner service is constructed immediately after, before any cooldown exists). A claimed second path through `backend/background_scanner.py:~308` was RETRACTED after peer review (2026-09-04): the `or {}` mints a new dict only when the config is empty, and an empty config has no discovery mode, so `rss_active` is false and the RSS service is not constructed there. Whether a settings save replaces `reg.config` with a new object is an open question for the HDE-6 fix, not a claim.
...
**HDE4-R1 (2026-09-05), #113 @ `436819d`.** The reviewer gave HDE-4 a conditional pass with no redesign and one correction: the two accounting reads went through a query helper that swallows every failure into an empty list, so a dead database read as a legitimate zero day. For accounting, zero is a fact and "unavailable" is a different state. The reads now use a strict helper that signals failure; both methods return None when the data cannot be read and real zeros when the database is healthy and empty; `GET /sources` keeps its fail-soft shape (None keys on unavailability, zeros on an empty day, 200 either way). Tests pin both sides (no connection; a query error raised below the helper; a healthy empty database), and the swallowing fallback restored on a copy fails the unavailability test. The source-health side finding is recorded as a separate policy decision, per the reviewer: reveal-accounting outcome and source-health outcome are different axes. Next: HDE-5 in the reviewer's order, starting in `backend/download_service.py`.

## 16. HDE-5: the HDEncode docstrings say what the code does (2026-09-05)

PR #114 @ `194ba8c`, **stacked on #113** (→ #109 → main), draft, unmerged, docs only. The reviewer's order and vocabulary followed; the round-7 writing rules applied.

| item | what was measured | what changed |
|---|---|---|
| docs-only | for every changed file, docstrings stripped from the committed and working versions and the syntax trees compared: identical, ten files (the checker first decoded git output as cp1252 and faked five differences; fixed to UTF-8) | nothing executable changed |
| the vocabulary file | `backend/download_service.py`: `scrape_links` claimed two hosts and a plain list return; `_log_page_diagnostics` named neither its stages nor diagnostic ownership; `download_item` did not distinguish the reveal-served flag from the delivery flag | rewritten mechanically with the code lines; the unit accounted is the reveal-boundary invocation; an observation does not imply a transport attempt; `refused` covers invocations stopped before transport; accounting is observer-only and guarded |
| separate axes | `source_health.py`: the recorder records stripped and served-other-host diagnostics as a health success, for any source (no source gate on that branch; today's only caller passes hdencode) | stated as current, intentional behaviour and an open policy decision, not a bug to fix by coupling the two systems |
| the invariant | coordinator, queue and hold code: reveal counts play no part in admission, cooldown, pacing, enable/disable, hold arm or release; grep confirms the table and read API are referenced only by the database module, the boundary write, the sources route and the tests | stated in the coordinator's module docstring (not the class docstring, which the open coordinator PR touches adjacently) and in the queue's five hold-related methods, naming the enforcing test |
| the database | observation-row semantics, UTC-day bucketing, one row per boundary invocation, the sha256 url as pseudonymous correlation, no retention code, the volume estimate, unavailable versus genuine zero by the strict read | documented at the table and the three methods |
| callers | `caller` is the consumer class, `context_id` the correlation id, neither policy; a retry that reaches the boundary produces another row, an attempt short-circuited by the dedup checks produces none | documented in the action service, the queue and the four glue surfaces, tersely |
| truth-check | a read-only lane checked twenty-six claims with file:line citations: all true; it found four cross-references made stale by the relocation, two overstatements (retries always producing rows; "blocked" listed as cooldown/challenge/rate limit only) and three surviving ticket ids | all corrected before commit |
| suites | twelve focused files in CI's collection order: 753 passed (this lineage lacks #111, so the TST-2 leak reproduces if the scrape-outcomes file runs before the queue-followups file in one process; CI's alphabetical order does not, and docstrings are inert at runtime); CI on `194ba8c`: green on Python 3.11, 3.12 and frontend | — |
| stack | seven-branch merge rehearsal on a scratch main, #112 before HDE-5: all clean, no conflicts (the coordinator paragraph was moved to the module docstring precisely because #112 touches the line after the class docstring); the integrated six-branch stack's full suite passed 5543 earlier today and HDE-5 adds no tests | — |

Process, per the owner's direction: docstrings by a Sonnet lane from the reviewer's spec; truth-check by a separate Sonnet lane after the Opus lane was cut off by the session limit; rewordings by a Sonnet lane; docs-only proof, the relocation decision and the merge rehearsal by the supervisor. With HDE-5 landed, the nine findings of the five-week review are all closed in code; the remaining items are the RSS hybrid with the listing canary (specified, not built) and TST-3 (FOUND, undiagnosed).

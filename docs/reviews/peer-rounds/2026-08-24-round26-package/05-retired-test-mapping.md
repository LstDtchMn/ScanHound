...
| Retired test | | Destination |
|---|---|---|
| `test_every_live_key_resolves_deterministically` | **A** | `TestTheExactLiveKeyMappingIsAsserted::test_each_live_key_maps_to_its_own_arm`. **Corrected in round 27 — the eighth overstated A.** Round 26 added the exact-mapping test but left this row pointing at `TestAttribution::test_apply_attributes_every_live_key`, which proves only that every row lands at *an* active revision. The retired test asserted the exact mapping; the direct destination is the new test, and a mutation swapping two arms kills it while the old destination still passes. |
...
| Retired test | | Destination |
|---|---|---|
| `test_all_three_live_keys_move` | **B** | `TestAttribution::test_apply_attributes_every_live_key`, **plus** `TestTheExactLiveKeyMappingIsAsserted`. **Reclassified in round 26 — the seventh overstated A.** The named destination asserts every live key ends at *an* active revision; it never asserts *which*. So swapping `hdencode:4k` and `hdencode:remux` still satisfies it — both are movie arms at active revisions, so the migration has still "moved all three keys", into the wrong arms. That swap is the mutation actually run, not a hypothetical. The retired test named the mapping. The round-26 companion pins the exact key→arm_id triple, and a mutation check confirms swapping two destinations fails it while the original test still passes. |
...
| `test_no_row_is_lost` | **B** | `TestTheShapeMigration::test_every_row_survives_untouched` compares per-row tuples, and `validate_shape_migration()` now enforces the same thing in PRODUCTION with seven dedicated tests. **Name corrected in round 26:** this row named `rebuild_equivalence_failure()`, which does not exist in the code — it was the name of the *first* attempt, discarded when review showed it compared plain tuples against `sqlite3.Row` and so would have refused every migration. A mapping entry that names a function a reader cannot find is unverifiable, which is the specific failure this whole document exists to prevent. |
...
| Disposition | Count |
|---|---|
| **A** — same path still exercised | 21 |
| **B** — strictly stronger replacement | 11 |
| **C** — intentionally obsolete, superseding rule named | 5 |
| **LOST** — no destination, found by review, now restored | 6 |
| **Total retired** | 43 |

Seven **A** entries have now been reclassified across rounds 22–26, each
found by deliberately looking for the *next* one rather than accepting that the
table had converged. Six of the seven were found this way; none surfaced on
their own. That is the argument for continuing to ask, not evidence that the
remaining entries are sound.

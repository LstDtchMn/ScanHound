...
1. **The frontend does not render any of this** — in the diff you are reading. `promotion` has been published since #108 and `frontend/src/routes/rss/+page.svelte` read none of it. Worse, the page's mode selector showed `status.mode`, the **requested** mode, so the UI could display "RSS primary" while the runtime ran shadow with the reason unread two keys away.

   **CLOSED after this document was first written**, on a separate branch: `feat/rss-canary-status-page` (PR #117, draft, stacked on this one) shows the running mode and why, follows the promotion authority rather than shadow readiness for the selector, and renders per-source canary health. Sixteen new unit tests on extracted helpers, `svelte-check` clean, the full frontend suite (476) green, production build succeeds — I had wrongly assumed I could not build the frontend here; the dependencies simply were not installed. Not verified rendered against a live backend. **Review it separately; it is not part of this diff.**
...
5. **A canary source that is never crawled promotes anyway, then demotes twelve hours later.** `background_scan_categories` can be narrowed to a subset (e.g. `["4k"]`); the contract's canary sources default to `4k, remux, tv`. Nothing at activation compared the two, so an operator could promote into a configuration guaranteed to go stale and auto-demote once `canary_max_age` passed.

   **CLOSED after this document was first written.** The owner asked for the guard, so it exists: `canary_source_not_crawled`, an ACTIVATION blocker only, on branch `feat/rss-canary-activation-guard` (PR to follow), stacked on this one. The runtime deliberately gains no new demotion path — a category switched off under a live promotion is still handled by the canary going stale, on observed evidence, which is the only thing allowed to demote. Six mutants, all killed. **Review it separately; it is not part of the diff you are reading**, and the question below is answered rather than open.
...
4. §8.1 and §8.5 became separate branches after this was written. Should either have been part of part 2 rather than stacked on it?
5. §8.5 — the owner chose "refuse it", and the guard is built on a separate branch. The question that remains is narrower: is *activation only* the right place, or does a category switched off under a live promotion deserve a faster answer than waiting for the canary to age out?

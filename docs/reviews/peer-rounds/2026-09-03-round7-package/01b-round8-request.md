...
2. Is the guard's snapshot (one level deep: the names inside each bucket and the size of loose files; no mtimes, because a bucket's mtime measurably does not move on Windows when a file is added inside it) the right instrument, or do you want it deeper? It is designed to be cheap enough to run before and after every one of 5,400 tests.

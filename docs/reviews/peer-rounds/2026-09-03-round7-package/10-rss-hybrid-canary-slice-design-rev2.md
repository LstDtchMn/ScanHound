...
**Blocking dependency, proven 2026-09-06.** No epoch can start today. The pinned hourly task `mount-nas-shares.ps1` (`C:\ProgramData\ScanHound\deploy`, the 2026-08-16 version, sha256 `7F0F2D29`, 954 lines) stops the container whenever it finds it running without the critical TV share mounted, to prevent a TV rename writing into a plain VM directory; its log records exactly that at 2026-09-05 21:51:47 local, 49 minutes after the container was restarted, and the `unless-stopped` policy makes the stop final. The app therefore cannot stay up for a full poll interval, so no shadow cycle is recorded. The remedy is the already-built pair — the share-identity write guard (#102) and the task change that leaves a guard-capable container running in degraded mode (#101) — merged, deployed, and the pinned file replaced; or the share returning. Both are owner decisions. Everything below describes the epoch once one of them holds.

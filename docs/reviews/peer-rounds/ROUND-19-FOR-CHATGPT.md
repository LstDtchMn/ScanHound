# ScanHound — Round 19 peer review

**Self-contained.** Everything you need is in this one file, including the
complete diff. Nothing is referenced that is not here.

You are reviewing an attestation-authority feature on the branch
`fix/round12-attestation-authority`. This is round 19 of an adversarial review
cycle; rounds 12 through 18 are closed. Your round-18 findings M18-1 through
M18-4 are all addressed below.

**Contents of this document**

1. Identity and live state
2. The request — each M18 finding, the gate item by item, and four questions
3. Evidence — commands, mutation results, suite figures, live data
4. Provenance — SHAs, blob hashes, container identity, what is NOT covered
5. The complete diff of `backend/` and `tests/` against `main`
...
# 5. Complete diff — `backend/` and `tests/` vs `main`

284269 bytes. This is the whole change; nothing is elided.

```diff
diff --git a/backend/api/routes/scanner.py b/backend/api/routes/scanner.py
index af7bd09..d012239 100644
...
diff --git a/backend/arms.py b/backend/arms.py
new file mode 100644
index 0000000..8fb710c
--- /dev/null
+++ b/backend/arms.py
@@ -0,0 +1,203 @@
...
diff --git a/backend/background_scanner.py b/backend/background_scanner.py
index a314d35..cfce19e 100644
--- a/backend/background_scanner.py
+++ b/backend/background_scanner.py
...
+from backend.arms import default_registry
 ...
 logger = logging.getLogger(__name__)
@@ -31,11 +32,59 @@ _ALL_CATEGORY_FLAGS = {
 }
...
@@ -562,28 +611,145 @@ class BackgroundScanner:
...
diff --git a/backend/coverage.py b/backend/coverage.py
new file mode 100644
index 0000000..596e067
--- /dev/null
+++ b/backend/coverage.py
@@ -0,0 +1,444 @@
...
index 496e622..af7d043 100644
--- a/backend/database.py
+++ b/backend/database.py
@@ -84,6 +84,26 @@ class DatabaseManager:
...
@@ -91,7 +111,13 @@ class DatabaseManager:
...
@@ -121,7 +147,16 @@ class DatabaseManager:
                 raise
...
+            logger.exception("could not close the revocation session")
         with self._lock:
             if self.conn:
                 try:
@@ -1214,6 +1249,94 @@ class DatabaseManager:
...
+                """)
                 for _column, _declaration in (
                     ("imdb_id", "TEXT"),
                     ("tmdb_id", "TEXT"),
@@ -4274,7 +4397,14 @@ class DatabaseManager:
...
@@ -4324,6 +4454,911 @@ class DatabaseManager:
...
@@ -4402,6 +5437,14 @@ class DatabaseManager:
         if not wanted:
             return {}
         out = {}
...
+        disabled = self._authority_disabled
         for start in range(0, len(wanted), 300):
             ...
             rows = self._query_dicts(
@@ -4409,12 +5452,32 @@ class DatabaseManager:
...
+
 ...
 from dataclasses import dataclass, field
@@ -264,6 +277,23 @@ class ScannerService:
...
@@ -371,6 +401,7 @@ class ScannerService:
...
@@ -403,6 +434,13 @@ class ScannerService:
             self.items.clear()
...
@@ -818,6 +856,9 @@ class ScannerService:
...
@@ -833,6 +874,24 @@ class ScannerService:
...
@@ -849,6 +908,29 @@ class ScannerService:
...
@@ -881,6 +963,11 @@ class ScannerService:
                     else:
...
@@ -941,6 +1028,11 @@ class ScannerService:
...
@@ -951,6 +1043,21 @@ class ScannerService:
...
@@ -975,6 +1082,7 @@ class ScannerService:
                         if not post_url:
                             continue
                         page_posts += 1
+                        source_posts += 1
...
@@ -987,6 +1095,54 @@ class ScannerService:
                         #
...
@@ -1046,13 +1202,26 @@ class ScannerService:
...
@@ -1077,7 +1246,45 @@ class ScannerService:
...
@@ -1123,6 +1330,8 @@ class ScannerService:
...
@@ -1142,6 +1351,17 @@ class ScannerService:
         else:
...
@@ -1207,10 +1427,12 @@ class ScannerService:
...
diff --git a/tests/test_media_kind_is_server_owned.py b/tests/test_media_kind_is_server_owned.py
index 73f3db6..105e977 100644
--- a/tests/test_media_kind_is_server_owned.py
+++ b/tests/test_media_kind_is_server_owned.py
...
diff --git a/tests/test_rescan_preserves_classification.py b/tests/test_rescan_preserves_classification.py
index a0b8082..e595074 100644
--- a/tests/test_rescan_preserves_classification.py
+++ b/tests/test_rescan_preserves_classification.py
...
diff --git a/tests/test_round12_attestation_authority.py b/tests/test_round12_attestation_authority.py
new file mode 100644
index 0000000..4e9ad94
--- /dev/null
+++ b/tests/test_round12_attestation_authority.py
@@ -0,0 +1,252 @@
...
diff --git a/tests/test_round13_hold_withdraws_identity.py b/tests/test_round13_hold_withdraws_identity.py
new file mode 100644
index 0000000..65b475f
--- /dev/null
+++ b/tests/test_round13_hold_withdraws_identity.py
@@ -0,0 +1,106 @@
...
+        assert _identity(db) == "tv_season"
diff --git a/tests/test_round13_parser_health_coverage.py b/tests/test_round13_parser_health_coverage.py
new file mode 100644
index 0000000..067590c
--- /dev/null
+++ b/tests/test_round13_parser_health_coverage.py
@@ -0,0 +1,157 @@
...
diff --git a/tests/test_round13_revocation_failclosed.py b/tests/test_round13_revocation_failclosed.py
new file mode 100644
index 0000000..f9dd7de
--- /dev/null
+++ b/tests/test_round13_revocation_failclosed.py
@@ -0,0 +1,157 @@
...
+        assert db.reconcile_unrevoked_conflicts() == 1
+        assert _served_kind(db) is None
diff --git a/tests/test_round14_listing_claim_ledger.py b/tests/test_round14_listing_claim_ledger.py
new file mode 100644
index 0000000..bdfa4d1
--- /dev/null
+++ b/tests/test_round14_listing_claim_ledger.py
@@ -0,0 +1,404 @@
...
+            "an inert writer")
diff --git a/tests/test_round15_restart_durability.py b/tests/test_round15_restart_durability.py
new file mode 100644
index 0000000..c0bc8ec
--- /dev/null
+++ b/tests/test_round15_restart_durability.py
@@ -0,0 +1,152 @@
...
+        finally:
+            db.close()
diff --git a/tests/test_round16_alias_and_ordering.py b/tests/test_round16_alias_and_ordering.py
new file mode 100644
index 0000000..29bcd2e
--- /dev/null
+++ b/tests/test_round16_alias_and_ordering.py
@@ -0,0 +1,155 @@
...
+        self._cycle(db, [])
+        assert _identity(db, RAW_A) == "movie"
diff --git a/tests/test_round16_coverage_evaluator.py b/tests/test_round16_coverage_evaluator.py
new file mode 100644
index 0000000..32790b1
--- /dev/null
+++ b/tests/test_round16_coverage_evaluator.py
@@ -0,0 +1,386 @@
...
diff --git a/tests/test_round16_journal_durability.py b/tests/test_round16_journal_durability.py
new file mode 100644
index 0000000..55aacd3
--- /dev/null
+++ b/tests/test_round16_journal_durability.py
@@ -0,0 +1,146 @@
...
+        finally:
+            second.close()
diff --git a/tests/test_round16_low_findings.py b/tests/test_round16_low_findings.py
new file mode 100644
index 0000000..6b1ea0d
--- /dev/null
+++ b/tests/test_round16_low_findings.py
@@ -0,0 +1,157 @@
...
+                                  _claim(URL, "tv", "tv")])
+        assert db.consume_cross_crawl_conflicts() == 1
diff --git a/tests/test_round16_traversal_emission.py b/tests/test_round16_traversal_emission.py
new file mode 100644
index 0000000..98d1cdc
--- /dev/null
+++ b/tests/test_round16_traversal_emission.py
@@ -0,0 +1,290 @@
...
diff --git a/tests/test_round18_arm_scope_and_snapshot.py b/tests/test_round18_arm_scope_and_snapshot.py
new file mode 100644
index 0000000..ce70dda
--- /dev/null
+++ b/tests/test_round18_arm_scope_and_snapshot.py
@@ -0,0 +1,370 @@
...
diff --git a/tests/test_round19_arm_key_migration.py b/tests/test_round19_arm_key_migration.py
new file mode 100644
index 0000000..d7a26aa
--- /dev/null
+++ b/tests/test_round19_arm_key_migration.py
@@ -0,0 +1,287 @@
...
diff --git a/tests/test_round19_one_arm_identity.py b/tests/test_round19_one_arm_identity.py
new file mode 100644
index 0000000..62be93f
--- /dev/null
+++ b/tests/test_round19_one_arm_identity.py
@@ -0,0 +1,319 @@

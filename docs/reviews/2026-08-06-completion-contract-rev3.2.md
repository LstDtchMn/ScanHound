...
(the round-13 closure series, the audit fix, and the round-14 R-4 binding). Base `main@7adb17b`;
...
| O-5 | Billing restored; pending heads CI-attested with EXECUTED steps | **CLOSED for this candidate 2026-08-04.** Root cause was not an outage: the repo was PRIVATE and burned 1,801+ of its 2,000 monthly Actions minutes (last real run 2026-08-03 13:56Z with 14/14/16 steps; every run after 14:20Z had **0 steps executed** — the job never started). Repo made public → unlimited minutes. Green runs with executed steps: main after PR #40 — https://github.com/LstDtchMn/ScanHound/actions/runs/30947333538 (16/12/12 steps, success); the COMBINED candidate — https://github.com/LstDtchMn/ScanHound/actions/runs/30948928368 (16/14/14 steps, success) | 🔒 / C / CG | ✅ |
| O-6 | Rebase done; dedupe verified; harness + full suite at the bound head with COMMITTED artifacts; validated as the exact COMBINED tree after main advanced | branch-only evidence as before (harness exit 0; mutation harness 10/10; clean-room suite 4793/0 at `70939859e620e8398ba95b6eb53c33abef1c308d`). **Combined tree** `agent/hybrid-sweep-combined` @ `f77ace7` = the reviewed branch + merge of main after PR #40: clean merge, and the coexistence that actually mattered is proven — PR #40's `CATEGORY_NONE_SENTINEL` and this branch's `_effective_category` compose in ONE expression in `_filter_and_sort`. Suite on the combined tree: **4797 passed / 0 failed / 4 skipped / exit 0** (four more than the branch alone — PR #40's sentinel suite now runs beside the authority tests), artifact `docs/reviews/evidence/2026-08-06-full-suite-combined-f77ace7.txt`, plus the green CI run in O-5. A MERGE onto a new branch, not a rebase: the rows above bind evidence to exact SHAs that a rebase would rewrite | C / harness+CI / 🔒 merge follows | ✅ |

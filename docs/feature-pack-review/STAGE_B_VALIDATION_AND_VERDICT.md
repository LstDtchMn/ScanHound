...
| **D — schema migration matrix (synthetic data)** | **PASS** — created a genuine pre-feature-pack DB with `origin/main`'s `DatabaseManager` (`user_version=2`, 16 tables, a seeded `downloads` row), then: (1) opened it with the feature-pack code → migrated to `user_version=6`, `integrity_check=ok`, seed row PRESERVED, feature tables (`hdencode_shadow_cycles`/`hdencode_actions`/`hdencode_candidates`) created — additive, no loss; (2) OLD-IMAGE REOPEN: `origin/main` v2 code reopens the migrated DB, reads data fine, re-stamps `user_version=2` — benign because migrations are `IF NOT EXISTS` + guarded `ALTER` (idempotent re-upgrade); (3) ROLLBACK: restoring the pre-migration backup returns a clean v2 (feature tables gone, data intact); (4) interrupt-safety is structural — migration commits in one transaction, so an interrupt before commit rolls back to the old version. |
| **D — migration matrix (real production data)** | NOT RUN — the safety classifier correctly blocks autonomous access to the production `crawler.db` / its backups (production-data protection). Needs a byte-for-byte snapshot Jesse provides, or an explicit tool-permission grant; the running production instance was not touched. The migration *machinery* is proven by the synthetic matrix above + the in-suite additive tests. |

...
def notify(title, message, priority):
    ...
    Returns True only when the push was CONFIRMED delivered (HTTP 200).
    Callers that deduplicate on a marker file must write the marker only on
    True: writing it after a failed send converts a transient Gotify outage
    into a permanently lost alert, because the dedup then suppresses every
    retry. (A rejected push returning 404 while curl exits 0 is exactly the
    dead-man's-switch lesson from the Kuma monitors.)
    """
    token = _gotify_token()
    if not token:
...
def main():
    ...
    if stop:
        ...
        # NOTIFY ONLY WHEN THE STOP SET CHANGES (Jesse, 2026-08-14). The full
        # detail still logs on EVERY run, above -- only the phone alert is
        # deduplicated. Four identical Gotify alerts a day taught the reader
        # that the alert is noise, which is how the stale schema pin rode along
        # unnoticed for four days; the next DIFFERENT condition would have
        # looked like the same noise. Same marker-file idiom as
        # gate-passed.notified below.
        #
    ...
    if ready:
        ...
        if not marker.exists():
            # Same lost-alert hole as the stop-condition marker had: this used
            # to write unconditionally, so one failed send would have silenced
            # the gate-passed alert forever.
            if notify("ScanHound QUAL: gate PASSED",

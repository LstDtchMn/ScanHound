...
- **Frontend:** `BulkBar`'s Archive button reuses the existing global `controlsDisabled` flag every other button in that component already uses, rather than a new per-selection "all selected jobs are `'applying'`" check — the latter would require `BulkBar` to receive full job objects instead of just ids, a real prop-shape change none of its other buttons need, for a safety guarantee the backend already provides by silently skipping `'applying'` jobs and returning the true count. Verify instead: a mixed-status selection archives only the eligible jobs and the toast reflects the true (possibly smaller) count. Selecting the Archived tab renders jobs of mixed underlying status together. Unarchive (single row and bulk) round-trips a job back into the default (non-archived) view.

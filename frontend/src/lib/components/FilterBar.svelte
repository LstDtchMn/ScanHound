<script lang="ts">
  ...
  async function bulkDownloadAll() {
    ...
    downloadingAll = true;
    try {
      await api.downloadBatch(selected.map(i => ({ url: i.url, title: i.title, year: i.year, season: i.season, resolution: i.resolution, size: i.size, hdr: i.hdr, dovi: i.dovi, category: i.category })), $downloadHost);

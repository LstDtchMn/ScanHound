<script lang="ts">
  ...
  import { login, refreshAuthStatus, hasPassword } from '$lib/stores/auth';
  ...
  // Fresh install / wiped-credential DB (SH-H01): the server fails closed
  // with no password/nonce set, so show the set-password prompt instead of
  // goto('/') — that goto is exactly the redirect loop this branch fixes,
  // since the app would immediately 401 and bounce back here.
  ...
  $effect(() => {
    // Once the fresh-install set-password form succeeds, hasPassword flips
    // true — reload into the app the same way a normal login does.
    if (setupRequired && $hasPassword && typeof window !== 'undefined') {
  ...
  {#if setupRequired}
    <div class="w-full max-w-sm space-y-4">
      <div class="text-center space-y-1">
        ...
        <p class="text-xs text-[var(--text-secondary)]">First-time setup — choose a password to protect this server.</p>
      </div>
      <ChangePassword />
    </div>
  {:else}
